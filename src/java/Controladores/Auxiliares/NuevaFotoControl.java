...
public class NuevaFotoControl extends Action{
    ...
    public void run() throws ServletException, IOException {
        ...
        String rutaLogo = application.getRealPath("/Imagenes/Personal/Fotos");
        UploadBean upbean = new UploadBean();

        try{
            MultipartFormDataRequest mrequest = new MultipartFormDataRequest(request);
            datos.put("fotoSel", mrequest.getParameter("fotoNueva"));
            datos.put("paso", "1");
            upbean.setFolderstore(rutaLogo);
            upbean.setOverwrite(true);
            upbean.store(mrequest,"fotoFile");
            
            //reducir la foto
            this.reducirFoto(application.getRealPath("/Imagenes/Personal/Fotos/"+mrequest.getParameter("fotoNueva")));
