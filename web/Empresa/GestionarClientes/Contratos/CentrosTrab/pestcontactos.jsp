    ...
    <tr>
        <td width="70%" align="right">
            <a id="btnBajaContac" href="javascript: BajaContactoClick()"
                style="width: 180px; font-weight: bold; color: #0B610B;" title="Dar de baja Contacto del Contrato">
                Baja de Contacto
            </a>
        </td>
        <td width="15%" align="right">
            <a id="btnEditarContac" href="javascript: EditarContactoClick()"
                style="width: 150px; font-weight: bold; color: #0B610B;" title="Editar Contacto del Contrato">
                Editar Contacto
            </a>
        </td>
        <td width="15%" align="right">
            <a id="btnNuevoContac" href="javascript: NuevoContactoClick()"
                style="width: 150px; font-weight: bold; color: #0B610B;" title="Agregar Contacto del Contrato">
                Nuevo Contacto
            </a>
        </td>
    </tr>
...
<script lang="javascript">
    ...
    /*function Selecciona(lista){
        var lstipos = document.getElementById('lstTiposMedios');
        var lstels = document.getElementById('lstMedios');
        var lsexts = document.getElementById('lstExtensiones');
        var pos = lista.selectedIndex;
        var dato = document.getElementById('datomedio');
        var tipo = lstipos.options[pos].value;
        if (tipo!='3' && lista.name=='lstMedios')
            dato.value = telefono(lista.options[pos].text);
        else
            dato.value = lista.options[pos].text;
    ...
    function BajaContactoClick(){
        ...
        frm.paginaSig.value = '/Empresa/GestionarClientes/Contratos/CentrosTrab/nuevocentro.jsp';
    ...
    function EditarContactoClick(){
        Espera();
        var frm = document.getElementById('frmNuevoCT');
        frm.paginaSig.value = '/Empresa/GestionarClientes/Contratos/CentrosTrab/nuevocontactocentro.jsp';
...
<table width="70%" align="center" cellpadding="5px">
    <tr>
        ...
        <td width="15%" align="right">
            <a id="btnEditarContac" href="javascript: ContactoClick(1)"
                style="width: 150px; font-weight: bold; color: #0B610B;" title="Editar Contacto del Contrato">
                Editar Contacto
            </a>
        </td>
        <td width="15%" align="right">
            <a id="btnNuevoContac" href="javascript: ContactoClick(0)"
                style="width: 150px; font-weight: bold; color: #0B610B;" title="Agregar Contacto del Contrato">
                Nuevo Contacto
            </a>
        </td>
    ...
    <tr>
        <td width="100%" align="center" colspan="3">
            <select id="lstContactos" name="lstContactos" class="lista" size="10"
                    style="width: 600px" onclick="MostrarAcciones()">
                <%for (int i=0; i < contactos.size(); i++){
                    ContactoContrato cc = contactos.get(i);
                %>
                    <option value="<%=cc.getId()%>">
                        <%=cc.getContacto().getTitulo()+" "+cc.getContacto().getNombreCompletoPorApellidos()+" - "+cc.getContacto().getCargo()%>
                    </option>
...
<script lang="javascript">
    ...
    function MostrarAcciones(){
        ...
        if (lista.selectedIndex>=0){
            ...
            txtTel.value = lsTel.options[sel].value!='&'?telefono(lsTel.options[sel].value):'';
            txtCel.value = lsCel.options[sel].value!='&'?telefono(lsCel.options[sel].value):'';
